...
{
	struct stm32_serial_device* uart = (struct stm32_serial_device*) dev->user_data;	/* 获得真实的UART设备对象*/

	if (!(dev->flag & RT_DEVICE_FLAG_ACTIVATED))				/* 判断设备是否已经激活了*/
	{
		if (dev->flag & RT_DEVICE_FLAG_INT_RX)					/* 如果是中断接收模式，初始化接收的缓冲区*/
		{
		...
		/* Enable USART */
		USART_Cmd(uart->uart_device, ENABLE);				/* 使能USART */

		dev->flag |= RT_DEVICE_FLAG_ACTIVATED;				/* 设置设备为激活状态*/
	}
...
static void rt_serial_enable_dma(DMA_Stream_TypeDef* dma_channel,rt_uint32_t address, rt_uint32_t size)
{
